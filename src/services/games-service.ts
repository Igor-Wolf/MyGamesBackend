...
export const topGamesAllTimeService = async () => {
    ...
    const response1 : any = await apiGames.get(`/games?key=${key}&ordering=-metacritic&page_size=20`);
    const response2 : any = await apiGames.get(`/games?key=${key}&ordering=-added&page_size=20`);
    
    const lista :any = [...new Set([...response1.data.results, ...response2.data.results])];
    ...
    if (response) {
        ...
        const mostAdded = list.filter((item : any) => item.metacritic >= 85 && item.rating >= 4.3)
        
         mostAdded.sort((a : any, b: any) => 
...
export const metacriticGamesService = async () => {
    ...
    const response : any = await apiGames.get(`/games?key=${key}&ordering=-metacritic&page_size=20`);
...
export const trendingGamesService = async () => {
    ...
    const response : any = await apiGames.get(`/games?key=${key}&dates=${datePast},${dataFuture}&ordering=-added&page_size=20`);
...
export const newReleasesService = async () => {
    ...
    const response : any = await apiGames.get(`/games?key=${key}&dates=${dateNow},${dataFuture}&ordering=released&page_size=20`);
...
export const searchGameService = async (game : string) => {
    ...
    const response : any = await apiGames.get(`/games?key=${key}&search=${game}&page_size=20`);
